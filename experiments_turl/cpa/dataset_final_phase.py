...
def cta_tokenize_tabert_col(column_data, config):
    column_index = str(column_data[0])
    table_title = str(column_data[1])
...
class CPATaBertDataset(Dataset):
    ...
    def __init__(self,
        ...
        if os.path.exists(f"../../data/turl_cpa_data/processed_datasets/cpa_{split}_{bert}_ml{max_length}_win{window_size}_preprocess{preprocess}_aug{aug}_mp{main_percentage}_tabert.pkl"):
            print("Loading already processed {} dataset".format(split))

            with open(f"../../data/turl_cpa_data/processed_datasets/cpa_{split}_{bert}_ml{max_length}_win{window_size}_preprocess{preprocess}_aug{aug}_mp{main_percentage}_tabert.pkl", "rb") as f:
                df_dict = pickle.load(f)
            self.df = df_dict
...
#                 table_name.append([row["table_id"], row["label_ids"]])
            ...
            with open(f"../../data/turl_cpa_data/processed_datasets/cpa_{split}_{bert}_ml{max_length}_win{window_size}_preprocess{preprocess}_aug{aug}_mp{main_percentage}_tabert.pkl", 'wb') as f:
                pickle.dump(self.df, f)
...
def cpa_tokenize_neighbor_nomain_columns(column_data, config):
    ...
    if config.split == 'train':
        path = f'../../data/turl_cpa_data/Train/' + table_title + ".json.gz"
    elif config.split == 'dev':
        path = f'../../data/turl_cpa_data/Validation/' + table_title + ".json.gz"
    else:
        path = f'../../data/turl_cpa_data/Test/' + table_title + ".json.gz"
...
class CPANoMainNeighborColumnDataset(Dataset):
    ...
    def __init__(self,
        ...
        # _neighbor
        if os.path.exists(f"../../data/turl_cpa_data/processed_datasets/cpa_{split}_{bert}_ml{max_length}_win{window_size}_preprocess{preprocess}_aug{aug}_mp{main_percentage}_nomain.pkl"):
            print("Loading already processed {} dataset".format(split))

            with open(f"../../data/turl_cpa_data/processed_datasets/cpa_{split}_{bert}_ml{max_length}_win{window_size}_preprocess{preprocess}_aug{aug}_mp{main_percentage}_nomain.pkl", "rb") as f:
                df_dict = pickle.load(f)
            self.df = df_dict
            ...
            with open(f"../../data/turl_cpa_data/processed_datasets/cpa_{split}_{bert}_ml{max_length}_win{window_size}_preprocess{preprocess}_aug{aug}_mp{main_percentage}_nomain.pkl", 'wb') as f:
                pickle.dump(self.df, f)
