...
class CPATaBertDataset(Dataset):
    ...
    def __init__(self,
        ...
        if os.path.exists(f"../../data/CPA/{folder}/processed_datasets/cpa_{split}_{bert}_ml{max_length}_win{window_size}_preprocess{preprocess}_aug{aug}_mp{main_percentage}_tabert.pkl"):
            print("Loading already processed {} dataset".format(split))
            
            with open(f"../../data/CPA/{folder}/processed_datasets/cpa_{split}_{bert}_ml{max_length}_win{window_size}_preprocess{preprocess}_aug{aug}_mp{main_percentage}_tabert.pkl", "rb") as f:
            ...
            with open(f"../../data/CPA/{folder}/processed_datasets/cpa_{split}_{bert}_ml{max_length}_win{window_size}_preprocess{preprocess}_aug{aug}_mp{main_percentage}_tabert.pkl", 'wb') as f:
...
class CPAColTaBertDataset(Dataset):
    ...
    def __init__(self,
        ...
        # cta_cta_tabert_bert-base-uncased-bs-16_ml-128_seed-42_ws-0_aug-None.pt
        if os.path.exists(f"../../data/CPA/{folder}/processed_datasets/cpa_{split}_{bert}_ml{max_length}_win{window_size}_preprocess{preprocess}_aug{aug}_mp{main_percentage}_col_tabert.pkl"):
            print("Loading already processed {} dataset".format(split))
            
            with open(f"../../data/CPA/{folder}/processed_datasets/cpa_{split}_{bert}_ml{max_length}_win{window_size}_preprocess{preprocess}_aug{aug}_mp{main_percentage}_col_tabert.pkl", "rb") as f:
                df_dict = pickle.load(f)
            self.df = df_dict
...
#                 table_name.append([row["table_id"], row["label_ids"]])
            ...
            with open(f"../../data/CPA/{folder}/processed_datasets/cpa_{split}_{bert}_ml{max_length}_win{window_size}_preprocess{preprocess}_aug{aug}_mp{main_percentage}_col_tabert.pkl", 'wb') as f:
                pickle.dump(self.df, f)
...
## Reng
### method: sum_neighbor
def cpa_tokenize_sum_neighbor_columns(column_data, config):
    max_len_avail = config.max_length - (3 + config.window_size * 2)
    max_len_other_col = int(max_len_avail * (1 - config.main_percentage) // (1 + config.window_size * 2))
    max_len_target_col = max_len_avail - int(max_len_other_col * (1 + config.window_size * 2))
    ...
    main_col_stats = df_table_statistics.loc[df_table_statistics.column_index == '0'].values[0][2]
    main_col_stats = " ".join(replace_special_tokens(main_col_stats).split())
    ...
    main_col_data = main_col_stats + " [VAL] " + " ".join(main_col_list)
...
class CPASumNeighborColumnDataset(Dataset):
    def __init__(self,
                 filepath: str,
                 split: str,
                 tokenizer: transformers.PreTrainedTokenizer,
        ...
        # _neighbor
        if os.path.exists("../../data/CPA/{}/processed_datasets/cpa_{}_{}_ml{}_win{}_preprocess{}_aug{}_mp{}_sum_neighbor.pkl".format(folder, split, bert, max_length, window_size, preprocess, aug, main_percentage)):
            print("Loading already processed {} dataset".format(split))

            with open("../../data/CPA/{}/processed_datasets/cpa_{}_{}_ml{}_win{}_preprocess{}_aug{}_mp{}_sum_neighbor.pkl".format(folder, split, bert, max_length, window_size, preprocess, aug, main_percentage), "rb") as f:
                df_dict = pickle.load(f)
            self.df = df_dict
            ...
            stat_path = '../cpa/{}_dict.pkl'.format(split)
            ...
            processed_cols = list(tqdm(pool.imap(partial(cpa_tokenize_sum_neighbor_columns, config=self), cols, chunksize=100),total=len(cols)))
            pool.close()
            pool.join()
            ...
            with open("../../data/CPA/{}/processed_datasets/cpa_{}_{}_ml{}_win{}_preprocess{}_aug{}_mp{}_sum_neighbor.pkl".format(folder, split, bert, max_length, window_size, preprocess, aug, main_percentage), 'wb') as f:
                pickle.dump(self.df, f)
...
### method: nomain_neighbor
def cpa_tokenize_neighbor_nomain_columns(column_data, config):
...
class CPANoMainNeighborColumnDataset(Dataset):
    ...
    def __init__(self,
        ...
        # _neighbor
        if os.path.exists("../../data/CPA/{}/processed_datasets/cpa_{}_{}_ml{}_win{}_preprocess{}_aug{}_mp{}_neighbor_nomain.pkl".format(folder, split, bert, max_length, window_size, preprocess, aug, main_percentage)):
            print("Loading already processed {} dataset".format(split))

            with open("../../data/CPA/{}/processed_datasets/cpa_{}_{}_ml{}_win{}_preprocess{}_aug{}_mp{}_neighbor_nomain.pkl".format(folder, split, bert, max_length, window_size, preprocess, aug, main_percentage), "rb") as f:
                df_dict = pickle.load(f)
            self.df = df_dict
            ...
            pool = Pool(processes=10)
            processed_cols = list(tqdm(pool.imap(partial(cpa_tokenize_neighbor_nomain_columns, config=self), cols, chunksize=100),total=len(cols)))
            pool.close()
            pool.join()
            ...
            with open("../../data/CPA/{}/processed_datasets/cpa_{}_{}_ml{}_win{}_preprocess{}_aug{}_mp{}_neighbor_nomain.pkl".format(folder, split, bert, max_length, window_size, preprocess, aug, main_percentage), 'wb') as f:
                pickle.dump(self.df, f)
...
class CPANeighborColumnDataset(Dataset):
    def __init__(self,
        ...
        # _neighbor
        if os.path.exists("../../data/CPA/{}/processed_datasets/cpa_{}_{}_ml{}_win{}_preprocess{}_aug{}_mp{}_neighbor.pkl".format(folder, split, bert, max_length, window_size, preprocess, aug, main_percentage)):
            print("Loading already processed {} dataset".format(split))

            with open("../../data/CPA/{}/processed_datasets/cpa_{}_{}_ml{}_win{}_preprocess{}_aug{}_mp{}_neighbor.pkl".format(folder, split, bert, max_length, window_size, preprocess, aug, main_percentage), "rb") as f:
                df_dict = pickle.load(f)
            self.df = df_dict
            ...
            with open("../../data/CPA/{}/processed_datasets/cpa_{}_{}_ml{}_win{}_preprocess{}_aug{}_mp{}_neighbor.pkl".format(folder, split, bert, max_length, window_size, preprocess, aug, main_percentage), 'wb') as f:
                pickle.dump(self.df, f)
...
## end Austin/Reng
## method: single_col
def cpa_tokenize_column_pair(column_data, config):
...
class CPASingleColumnDataset(Dataset):
    def __init__(self,
        ...
        if os.path.exists("../../data/CPA/{}/processed_datasets/cpa_{}_{}_ml{}_win{}_preprocess{}_aug{}_mp{}_singlecolumn.pkl".format(folder, split, bert, max_length, window_size, preprocess, aug, main_percentage)):
            print("Loading already processed {} dataset".format(split))

            with open("../../data/CPA/{}/processed_datasets/cpa_{}_{}_ml{}_win{}_preprocess{}_aug{}_mp{}_singlecolumn.pkl".format(folder, split, bert, max_length, window_size, preprocess, aug, main_percentage), "rb") as f:
                df_dict = pickle.load(f)
            self.df = df_dict
            ...
            pool = Pool(processes=10)
            processed_cols = list(tqdm(pool.imap(partial(cpa_tokenize_column_pair, config=self), cols, chunksize=100),total=len(cols)))
            pool.close()
            pool.join()
            ...
            with open("../../data/CPA/{}/processed_datasets/cpa_{}_{}_ml{}_win{}_preprocess{}_aug{}_mp{}_singlecolumn.pkl".format(folder, split, bert, max_length, window_size, preprocess, aug, main_percentage), 'wb') as f:
                pickle.dump(self.df, f)
